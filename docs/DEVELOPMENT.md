...
> Примеры ниже показаны на `jarvis-macos`/`mac_*` — на Windows тот же приём применяется к `jarvis-windows`/`win_*`
> (`tests/test_plugins_windows.py`), на Linux — к `jarvis-linux`/`linux_*` (`tests/test_plugins_linux.py`).
> Три плагина независимы и живут в `plugins/jarvis-{macos,windows,linux}/`, но имеют одинаковую структуру
> файлов (`plugin.yaml`, `__init__.py`, `schemas.py`, `tools.py`, платформенный хелпер `mac.py`/`win.py`/`linux.py`).
