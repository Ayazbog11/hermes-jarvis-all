...
**Что уникальной архитектуры Kuni сознательно не взято и почему**: (a) диалог через личный Telegram-аккаунт
(tdlib userbot) — нарушает ToS Telegram и требует привязки живого аккаунта; JARVIS отправляет через
официальный `hermes send` (Bot API/webhook-адаптеры Hermes), это принципиально другая, легитимная модель
доступа; (b) транспарентный OpenAI-совместимый прокси-сервер — у Hermes Agent уже есть собственный
API-сервер (`API_SERVER_ENABLED`, порт 8642) с OpenAI-совместимым фасадом, второй прокси поверх него был
