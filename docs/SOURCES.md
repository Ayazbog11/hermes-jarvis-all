...
| https://github.com/alex2772/kuni | Раунд 7: точечно перенесены 4 идеи — отправка файлов/голосовых по команде (`messenger.send_file`/`jarvis_voice_note`), working memory на 1-3 дня (`working_memory.py`, аналог `data/working_memory.md`), Prometheus-метрики `/metrics` на HUD (аналог `llm_usage_*`). Архитектура (tdlib userbot, RAG-дневник со sleep-consolidation, свой Docker-стек SD/Whisper/OmniVoice, транспарентный LLM-прокси) не перенесена — см. `docs/RESEARCH.md` Раунд 7 для полного разбора что взято/не взято и почему |
