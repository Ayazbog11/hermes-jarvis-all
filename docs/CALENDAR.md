...
- **Outlook (COM)** — было на Windows в ранних версиях JARVIS, **удалено полностью** (см.
  `docs/CHANGELOG.md`, 2.1.0): требовал установленный и настроенный классический Microsoft Outlook
  (у многих его просто нет — Gmail/Google Workspace, встроенная «Почта», веб-Outlook без
  десктоп-клиента), а первое же обращение к COM-объекту `Outlook.Application` без настроенного
  профиля открывало мастер регистрации/окно первого запуска Outlook — причём HUD дёргал этот код
  каждые пару минут для виджета «Сегодня», так что окно всплывало заново и заново без действий
  пользователя. Заменён Google Calendar'ом без побочных эффектов.
