...
> Диаграмма и примеры ниже показаны для macOS (`jarvis-macos`, `mac_*`). Структура и весь остальной код
> (Hermes, `jarvis-core`, `jarvis-brain`, HUD, gateway, hooks) полностью общие для трёх платформ — на Windows
> вместо блока `jarvis-macos` подключается `jarvis-windows` (`win_*`, PowerShell/WMI/COM), на Linux —
> `jarvis-linux` (`linux_*`, wmctrl/xdotool/pactl/nmcli/…). PluginManager выбирает нужный плагин по ОС во
> время установки (`install.sh`/`install.ps1`/`install.linux.sh`), сам Hermes и остальные плагины не меняются.
