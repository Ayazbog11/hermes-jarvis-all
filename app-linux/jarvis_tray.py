#!/usr/bin/env python3
"""
JARVIS — значок в системном трее Linux (аналог app/JarvisMenuBar.swift на macOS
и app-windows/jarvis_tray.pyw на Windows).

Что делает:
  • иконка в трее (через pystray — на GNOME нужно расширение AppIndicator,
    на KDE/XFCE/большинстве DE работает из коробки): цвет статуса
    (HUD/gateway работают/недоступны/идёт обновление);
  • меню: открыть HUD, голосовой чат в терминале, старт/стоп сервисов,
    база знаний, обновление, настройки, логи, диагностика;
  • раз в 5 минут — проверяет статус сервисов и update.json (его пишет
    update.py auto, запускаемый systemd --user таймером);
  • при доступном обновлении — пункт «Обновить до X» и уведомление (notify-send).
Всё «тяжёлое» делает bin/jarvis (bash) — этот трей лишь удобная кнопка над ним.

Зависимости (ставятся install.linux.sh в venv Hermes): pystray, pillow.
На GNOME для видимости иконки в трее нужно расширение
"AppIndicator and KStatusNotifierItem Support" (extensions.gnome.org).

Запуск:  python3 app-linux/jarvis_tray.py
"""
...
try:
    ...
    sys.stderr.write(
        ...
        "(install.linux.sh ставит их автоматически в venv Hermes; запустите вручную для разработки)\n"
        "На GNOME также нужно расширение AppIndicator, иначе иконка не появится в верхней панели.\n"
    )
    raise
...
JARVIS_BIN = shutil.which("jarvis") or str(Path.home() / ".local" / "bin" / "jarvis")
...
def open_terminal(command: str) -> None:
    """Открыть интерактивную команду в новом окне терминала (голосовой чат и т.п.).

    Не существует единого терминала на Linux — пробуем самые распространённые по очереди.
    """
    env_cmd = f'HERMES_HOME="{HERMES_HOME}" bash -c "{command}; exec bash"'
    terminals = [
        ["x-terminal-emulator", "-e", "bash", "-c", env_cmd],
        ["gnome-terminal", "--", "bash", "-c", env_cmd],
        ["konsole", "-e", "bash", "-c", env_cmd],
        ["xfce4-terminal", "-e", f"bash -c '{env_cmd}'"],
        ["xterm", "-e", f"bash -c '{env_cmd}'"],
    ]
    for term in terminals:
        if shutil.which(term[0]):
            try:
                ...
                continue
    notify("JARVIS", "Не найден эмулятор терминала — установите gnome-terminal, konsole или xterm.")
...
def notify(title: str, body: str) -> None:
    ...
    if shutil.which("notify-send"):
        subprocess.Popen(["notify-send", "-a", "JARVIS", title, body])


def ask_text(title: str, prompt: str) -> str | None:
    """Простое модальное окно ввода текста — tkinter, стандартная библиотека, есть почти везде."""
...
def info_choice(title: str, text: str, buttons: list[str]) -> int:
    return 0 if confirm(title, text) else 1
...
def first_run_wizard(force: bool = False) -> None:
    ...
    if info_choice("Добро пожаловать. Я JARVIS.",
                   "Я живу в системном трее (или в верхней панели, если это GNOME).\n\n"
                   "Сейчас за минуту проверим, что всё готово: модель, системные утилиты, папка для ваших файлов.",
                   ["Проверить", "Позже"]) != 0:
        return
    ...
    if info_choice("Права и системные утилиты",
                   "Чтобы делать скриншоты, слышать буфер обмена и управлять окнами, нужны некоторые "
                   "системные утилиты (xdotool/wmctrl на X11 или grim/wl-clipboard на Wayland). Проверка займёт ~20 секунд.",
    ...
    choice = info_choice("Ваши файлы",
                         "Папка ~/JARVIS — моё хранилище: кладите туда документы, PDF, заметки, "
    ...
    if choice == 0 and shutil.which("xdg-open"):
        subprocess.Popen(["xdg-open", str(Path.home() / "JARVIS")])
...
def _perms(icon, item):
    """Единого центра разрешений на Linux нет — 'jarvis perms' печатает, каких утилит для текущего
    окружения (X11/Wayland) не хватает; открываем в терминале, чтобы вывод было видно (как doctor/logs)."""
    open_terminal("jarvis perms")
...
def build_menu() -> pystray.Menu:
    ...
    items += [
        ...
        pystray.MenuItem("Голосовой чат в терминале", _open_voice),
        ...
        pystray.MenuItem("Папка файлов ~/JARVIS", _open_vault),
        pystray.Menu.SEPARATOR,
...
def main() -> int:
    global _icon
    if not IS_LINUX:
        print("jarvis_tray.py предназначен только для Linux", file=sys.stderr)
        return 1
