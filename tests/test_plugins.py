...
def test_all_schemas_well_formed():
    ...
    for s in (core.schemas.JARVIS_HUD, core.schemas.JARVIS_TIMER, core.schemas.JARVIS_MODE, core.schemas.JARVIS_WEATHER,
              core.schemas.JARVIS_CALENDAR, core.schemas.JARVIS_SEND_MESSAGE, core.schemas.JARVIS_TELEGRAM):
        _check_schema(s)
...
def test_watchdog_focus_sync(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(wd, "notify", lambda *a: None)
    monkeypatch.setattr(wd, "battery_state", lambda: (None, False))
    # Тест проверяет разбор формата macOS DoNotDisturb DB (fallback-путь current_focus()) и должен
    # быть детерминирован независимо от реальной ОС/пакетов на машине, где гоняются тесты: на Linux
    # с установленными gsettings-desktop-schemas current_focus() иначе ушёл бы в GNOME-ветку раньше
    # macOS fallback'а — явно отключаем её здесь.
    monkeypatch.setattr(core.pc, "IS_LINUX", False)
    monkeypatch.setattr(core.pc, "IS_WINDOWS", False)
...
def test_selftest_classifier(monkeypatch):
    import importlib.util
    spec = importlib.util.spec_from_file_location("selftest", "scripts/selftest.py")
    st = importlib.util.module_from_spec(spec); spec.loader.exec_module(st)
    # Скрипт теперь общий для macOS/Windows/Linux и веткуется по IS_WINDOWS/IS_LINUX (реальным флагам
    # ОС хоста) — форсируем ветку macOS, т.к. эти тесты гоняются и на Linux/Windows CI.
    monkeypatch.setattr(st, "IS_WINDOWS", False)
    monkeypatch.setattr(st, "IS_LINUX", False)
    monkeypatch.setattr(st, "IS_MAC", True)
    ...
    # CANDIDATES строится один раз при импорте по реальной ОС хоста (PLUGIN_NAME), поэтому
    # на Linux/Windows CI здесь будет jarvis-linux/jarvis-windows — сверяем с самим PLUGIN_NAME,
    # а не жёстко с "jarvis-macos".
    assert st.CANDIDATES[0].name == st.PLUGIN_NAME and "plugins" in str(st.CANDIDATES[0])
    assert st.classify({"success": False, "error": "что-то странное"})[0] == "fail"
    assert all(h in load_plugin("jarvis-macos").tools.HANDLERS for h, _, _ in st.CHECKS_MAC)
...
def test_screen_context_detects_phrases_and_injects(monkeypatch):
    ...
    # без права на запись экрана — объясняем, как выдать (текст зависит от ОС)
    monkeypatch.setattr(core, "capture_screen", lambda: None)
    out = core.hook_pre_llm_call(session_id="s", user_message="что на экране?")
    assert ("Запись экрана" in out["context"]) or ("PowerShell" in out["context"]) or ("снимка экрана" in out["context"])
