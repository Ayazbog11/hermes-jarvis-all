...
def notify(title: str, text: str) -> None:
    """Уведомление ОС + событие на HUD (оба — best effort)."""
    if sys.platform == "darwin":
        subprocess.run(["osascript", "-e", f'display notification "{text}" with title "{title}"'],
                       capture_output=True, timeout=5)
    elif IS_WINDOWS:
        esc_t, esc_m = title.replace("'", "''"), text.replace("'", "''")
        subprocess.Popen(
        ...
        subprocess.run(["notify-send", "-a", "JARVIS", title, text], capture_output=True, timeout=5)
    try:
...
def download_and_extract(tarball: str, workdir: Path) -> Path:
    ...
    for cand in workdir.iterdir():
        if cand.is_dir() and (cand / INSTALLER_NAME).exists():
            return cand
    raise RuntimeError(f"в архиве нет {INSTALLER_NAME}")


def smoke_test(src: Path) -> None:
    """Все .py компилируются, VERSION есть, установщик синтаксически корректен — до того, как трогать установку."""
    ...
    if IS_WINDOWS:
        proc = subprocess.run(
            ["powershell.exe", "-NoLogo", "-NoProfile", "-NonInteractive", "-Command",
             f"$e = $null; [System.Management.Automation.Language.Parser]::ParseFile('{src / 'install.ps1'}', [ref]$null, [ref]$e); "
             "if ($e) { exit 1 } else { exit 0 }"],
            capture_output=True, timeout=30, creationflags=subprocess.CREATE_NO_WINDOW,
        )
        if proc.returncode != 0:
            raise RuntimeError("install.ps1 содержит синтаксические ошибки")
    elif IS_LINUX:
        subprocess.run(["bash", "-n", str(src / "install.linux.sh")], check=True)
    else:
        subprocess.run(["bash", "-n", str(src / "install.sh")], check=True)


def restart_services() -> None:
    """launchd-агенты (macOS) перезапускаем kickstart'ом; на Windows — Scheduled Task перезапуском."""
    if IS_WINDOWS:
        jarvis = Path(os.environ.get("LOCALAPPDATA", "")) / "hermes" / "bin" / "jarvis.ps1"
        try:
            subprocess.run(["schtasks", "/End", "/TN", "JARVIS-HUD"], capture_output=True, timeout=10)
            subprocess.run(["schtasks", "/Run", "/TN", "JARVIS-HUD"], capture_output=True, timeout=10)
            subprocess.run(["schtasks", "/End", "/TN", "JARVIS-Gateway"], capture_output=True, timeout=10)
            subprocess.run(["schtasks", "/Run", "/TN", "JARVIS-Gateway"], capture_output=True, timeout=10)
        except (OSError, subprocess.SubprocessError):
            pass  # перезапуск — best effort
        return
    if IS_LINUX:
        try:
            subprocess.run(["systemctl", "--user", "restart", "jarvis-hud.service"], capture_output=True, timeout=15)
            subprocess.run(["systemctl", "--user", "restart", "jarvis-gateway.service"], capture_output=True, timeout=15)
        except (OSError, subprocess.SubprocessError):
            jarvis = Path.home() / ".local" / "bin" / "jarvis"
            if jarvis.exists():
                subprocess.run([str(jarvis), "hud", "restart"], capture_output=True, timeout=20)
                subprocess.run([str(jarvis), "gateway", "restart"], capture_output=True, timeout=20)
        return
    if sys.platform != "darwin":
        return
    uid = os.getuid()
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis-update-") as tmp:
        ...
        if IS_WINDOWS:
            cmd = ["powershell.exe", "-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
                   str(src / "install.ps1"), "-Yes", "-NoScheduledTask", "-NoVoice", "-NoCron", "-NoApp"]
        elif IS_LINUX:
            cmd = ["bash", str(src / "install.linux.sh"), "--yes", "--no-systemd-user", "--no-system-packages",
                   "--no-voice", "--no-cron"]
        else:
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(f"{INSTALLER_NAME} завершился с ошибкой — выполнен откат.\n" + (proc.stderr or proc.stdout)[-1500:])
        log.append(f"✔ {INSTALLER_NAME} выполнен")
    new = installed()
