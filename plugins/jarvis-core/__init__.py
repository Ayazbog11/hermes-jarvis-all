...
def _battery_brief() -> str:
    return pc.battery_brief()


def _frontmost() -> str:
    return pc.frontmost_app()
...
def capture_screen() -> str | None:
    """Скриншот активного дисплея в $HERMES_HOME/cache/jarvis/screenshots (тот же каталог, что у mac_screenshot/win_screenshot)."""
    out_dir = Path(state.hermes_home()) / "cache" / "jarvis" / "screenshots"
    return pc.capture_screen(out_dir)
...
def screen_context(user_message: str) -> str:
    ...
    if not path:
        if pc.IS_MAC:
            return ("[JARVIS screen] Пользователь говорит о своём экране, но снять скриншот не удалось "
                     "(нет права «Запись экрана»). Попроси выдать право: Системные настройки → Конфиденциальность → "
                     "Запись экрана → терминал, или jarvis selftest --fix.")
        if pc.IS_WINDOWS:
            return ("[JARVIS screen] Пользователь говорит о своём экране, но снять скриншот не удалось. "
                     "Попроси проверить, что PowerShell может запускаться (jarvis doctor --fix).")
        return ("[JARVIS screen] Пользователь говорит о своём экране, но снять скриншот не удалось. "
                "Попроси проверить, что установлен инструмент для снимка экрана (grim/gnome-screenshot/scrot/import) "
                "и, на Wayland, что сессия поддерживает захват экрана (jarvis doctor --fix).")
...
def _run_shortcut(name: str) -> bool:
    """Запустить именованную автоматизацию (macOS Shortcut / Windows .ps1), если она есть. Никогда не бросает."""
    return pc.run_shortcut(name)
            ...
            if focus is not None and focus != self._last_focus:
                if self._last_focus is not None or focus:  # первый тик с «нет фокуса» — не событие
                    ...
                    if mapped and mapped != state.get_mode():
                        ...
                        _hud.emit("mode.set", {"mode": mapped, "source": "os-focus", "focus": focus})
                        sent.append(f"focus:{focus}->{mapped}")
                self._last_focus = focus
        return sent

    # ── режим "не беспокоить" (macOS Focus / Windows Focus Assist) ──
    @classmethod
    def map_focus(cls, focus_name: str) -> str | None:
        return pc.map_focus(focus_name)

    @staticmethod
    def macos_focus() -> str | None:
        """Имя активного режима "не беспокоить" ("" — нет); None — платформа не поддерживает."""
        return pc.current_focus()
